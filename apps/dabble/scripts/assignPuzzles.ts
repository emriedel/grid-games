/**
 * Dabble Puzzle Assignment Script
 *
...
import type { PoolPuzzle } from './generatePuzzles';
...
interface MonthlyAssignedFile {
  gameId: string;
  baseDate: string;
  puzzles: Record<string, PoolPuzzle>; // Key: puzzle number (string), Value: full puzzle data
}

// Must match the base date in storage.ts and config.ts
const BASE_DATE = '2026-02-01';
const GAME_ID = 'dabble';
 ...
 * Puzzle 1 = BASE_DATE (2026-02-01), so puzzle 1 is in 2026-02.
 */
...
async function main() {
  ...
  if (!fs.existsSync(poolPath)) {
    console.error('Error: pool.json not found. Run generatePuzzles.ts first.');
    process.exit(1);
  }
  ...
  for (let num = 1; num <= targetCount; num++) {
    ...
    if (availableIndex >= availablePuzzles.length) {
      ...
      console.warn(`Generate more puzzles with: npx tsx scripts/generatePuzzles.ts`);
      break;
    }
    ...
    console.log(`  Assigned #${num} (${dateStr}) → ${puzzle.id} (${puzzle.archetype}) [${month}]`);
  }
