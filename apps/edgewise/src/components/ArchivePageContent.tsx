...
import { PUZZLE_BASE_DATE_STRING } from '@/constants/gameConfig';
...
export function ArchivePageContent() {
  ...
  useEffect(() => {
    async function loadPuzzleIds() {
      ...
      try {
        ...
        console.warn('[edgewise] Failed to load puzzle IDs for archive:', error);
      } finally {
        setIsLoading(false);
      }
    }
  ...
  // For Edgewise, we show whether the puzzle was solved (as "stars")
  // 1 star = solved, 0 = not solved or failed
  const getPuzzleSolvedStatus = useCallback((puzzleNumber: number): number => {
    const currentPuzzleId = puzzleIds.get(puzzleNumber);
    const savedPuzzleId = getSavedPuzzleId(puzzleNumber);

    // Only return status if puzzleIds match
    ...
    // Return 1 if solved, 0 if failed
    return state.data.solved ? 1 : 0;
  }, [puzzleIds]);

  // Get number of guesses used for display
  const getPuzzleGuesses = useCallback((puzzleNumber: number): number | null => {
    const currentPuzzleId = puzzleIds.get(puzzleNumber);
    const savedPuzzleId = getSavedPuzzleId(puzzleNumber);

    // Only return guesses if puzzleIds match
    if (currentPuzzleId !== undefined && savedPuzzleId !== currentPuzzleId) {
      return null;
    }
  ...
  return (
    <ArchivePage
      ...
      getPuzzleStars={getPuzzleSolvedStatus}
      getPuzzleScore={getPuzzleGuesses}
