...
export function ArchivePageContent() {
  ...
  const archiveEntries = useMemo(() => {
    ...
    for (let num = todayPuzzleNumber - 1; num >= 1; num--) {
      ...
      // Check if solved (for completed puzzles)
      let wasSolved = false;
      if (isCompleted) {
        const state = findPuzzleState(num);
        wasSolved = state?.data?.solved ?? false;
      }
  ...
  return (
    <div className="min-h-screen bg-[var(--background,#0a0a0a)] flex flex-col items-center">
      ...
      <div className="w-full max-w-md mx-auto flex items-center justify-between px-4 py-3 border-b border-[var(--border,#27272a)]">
        {/* Menu button */}
        <HamburgerMenu currentGameId="edgewise" />
                  ...
                  >
                    ...
                    {entry.isCompleted ? (
                      entry.wasSolved ? (
                        ...
                        <X size={18} className="text-[var(--danger,#ef4444)]" />
                      )
                    ) : entry.isInProgress ? (
